...
class MunicipalityRepositoryImpl(
    ...
    CriteriaRepository<Municipality> {
    override fun getEntityClass(): Class<Municipality> = Municipality::class.java
    ...
    override fun findLargeMunicipalities(
        ...
        query
            .multiselect(
                municipality.get(Municipality_.name),
                municipality.get(Municipality_.code),
                municipality.get(Municipality_.population),
                municipality.get(Municipality_.area),
    ...
    override fun findByMinimumWards(
        ...
        query
            .multiselect(
                municipality.get(Municipality_.code),
                municipality.get(Municipality_.name),
                municipality.get(Municipality_.totalWards),
            ).where(
        ...
        val total =
            executeCountQuery(cb) { root ->
                cb.greaterThanOrEqualTo(root.get(Municipality_.totalWards), minWards)
            }
    ...
    private fun mapTupleToMunicipality(tuple: Tuple): Municipality =
        Municipality().apply {
            ...
            tuple.elements.getOrNull(3)?.let {
                when (it.alias) {
                    "distance" -> distance = it.javaType as Double
